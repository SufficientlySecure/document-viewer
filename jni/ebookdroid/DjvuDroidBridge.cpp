...
extern "C" jlong
Java_org_ebookdroid_djvudroid_codec_DjvuContext_create(JNIEnv *env, jclass cls)
{
...
extern "C" void
Java_org_ebookdroid_djvudroid_codec_DjvuContext_free(JNIEnv *env,
                                    jclass cls,
                                    jlong contextHandle)
{
...
jint* get_djvu_hyperlink_area(ddjvu_pageinfo_t *page_info, miniexp_t sexp, int &type, int &len)
{
    ...
    if(miniexp_car(iter) == miniexp_symbol("oval"))
    	type = 2;
    else
    if (miniexp_car(iter) == miniexp_symbol("poly"))
    	type = 3;
    else
	return NULL;
    ...
    {
	if (!number_from_miniexp (miniexp_car(iter), &x)) break;
	iter = miniexp_cdr(iter);
	array[i++] = (jint)x;
	if(i>=len) break;
    }
...
jobject get_djvu_hyperlink_mapping(JNIEnv *jenv, ddjvu_document_t* djvu_document, ddjvu_pageinfo_t *page_info, miniexp_t sexp)
{
    miniexp_t iter;
...
extern "C" jlong
Java_org_ebookdroid_djvudroid_codec_DjvuDocument_open(JNIEnv *env,
                                    jclass cls,
                                    jlong contextHandle,
                                    jstring fileName)
{
...
extern "C" void
Java_org_ebookdroid_djvudroid_codec_DjvuContext_handleMessage(JNIEnv *env,
                                    jobject thiz,
                                    jlong contextHandle)
{
...
extern "C" jlong
Java_org_ebookdroid_djvudroid_codec_DjvuDocument_getPage(JNIEnv *env,
                                    jclass cls,
                                    jlong docHandle,
                                    jint pageNumber)
{
...
extern "C" jobject
Java_org_ebookdroid_djvudroid_codec_DjvuDocument_getPageLinks(JNIEnv *env,
                                    jclass cls,
                                    jlong docHandle,
                                    jint pageNumber)
{
...
extern "C" jint
Java_org_ebookdroid_djvudroid_codec_DjvuDocument_getPageInfo(JNIEnv *env,
                                    jclass cls,
                                    jlong docHandle,
                                    jint pageNumber,
                                    jlong contextHandle,
...
extern "C" void
Java_org_ebookdroid_djvudroid_codec_DjvuDocument_free(JNIEnv *env,
                                    jclass cls, jlong docHandle)
{
    ddjvu_document_release((ddjvu_document_t*)docHandle);
}

extern "C" jint
Java_org_ebookdroid_djvudroid_codec_DjvuDocument_getPageCount(JNIEnv *env,
                                    jclass cls, jlong docHandle)
{
...
extern "C" jboolean
Java_org_ebookdroid_djvudroid_codec_DjvuPage_isDecodingDone(JNIEnv *env,
                                    jclass cls, jlong pageHandle)
{
	return ddjvu_page_decoding_done((ddjvu_page_t*)pageHandle);
}

extern "C" jint
Java_org_ebookdroid_djvudroid_codec_DjvuPage_getWidth(JNIEnv *env,
                                    jclass cls, jlong pageHangle)
{
	return ddjvu_page_get_width((ddjvu_page_t*)pageHangle);
}

extern "C" jint
Java_org_ebookdroid_djvudroid_codec_DjvuPage_getHeight(JNIEnv *env,
                                    jclass cls, jlong pageHangle)
{
...
extern "C" jboolean
Java_org_ebookdroid_djvudroid_codec_DjvuPage_renderPage(JNIEnv *env,
                                    jclass cls,
                                    jlong pageHangle,
                                    jint targetWidth,
                                    jint targetHeight,
...
extern "C" jboolean
Java_org_ebookdroid_djvudroid_codec_DjvuPage_renderPageBitmap(JNIEnv *env,
                                    jclass cls,
                                    jlong pageHangle,
                                    jint targetWidth,
                                    jint targetHeight,
...
extern "C" void
Java_org_ebookdroid_djvudroid_codec_DjvuPage_free(JNIEnv *env,
                                    jclass cls, jlong pageHangle)
{
...
//Outline
extern "C" jlong Java_org_ebookdroid_djvudroid_codec_DjvuOutline_open(JNIEnv *env,
                                    jclass cls, jlong docHandle)
{
//        DEBUG_PRINT("DjvuOutline.open(%p)",docHandle);
	...
	if (outline && outline != miniexp_dummy)
	{
		...
		return (jlong)outline;
//	    debug_outline(outline);
	}
	return 0;
}

extern "C" jboolean Java_org_ebookdroid_djvudroid_codec_DjvuOutline_expConsp(JNIEnv *env,
                                    jclass cls, jlong expr)
{
//        DEBUG_PRINT("DjvuOutline.expConsp(%p)",expr);
	return miniexp_consp((miniexp_t)expr);
}

extern "C" jstring Java_org_ebookdroid_djvudroid_codec_DjvuOutline_getTitle(JNIEnv *env,
                                    jclass cls, jlong expr)
{
//        DEBUG_PRINT("DjvuOutline.getTitle(%p)",expr);
        miniexp_t s = miniexp_car((miniexp_t)expr);
        if (miniexp_consp(s) && miniexp_consp(miniexp_cdr(s)) &&
	    miniexp_stringp(miniexp_car(s)) && miniexp_stringp(miniexp_cadr(s)))
        {
...
extern "C" jstring Java_org_ebookdroid_djvudroid_codec_DjvuOutline_getLink(JNIEnv *env,
                                    jclass cls, jlong expr, jlong docHandle)
{
...
extern "C" jlong Java_org_ebookdroid_djvudroid_codec_DjvuOutline_getNext(JNIEnv *env,
                                    jclass cls,jlong expr)
{
...
extern "C" jlong Java_org_ebookdroid_djvudroid_codec_DjvuOutline_getChild(JNIEnv *env,
                                    jclass cls, jlong expr)
{
//    DEBUG_PRINT("DjvuOutline.getChild(%p)",expr);
    miniexp_t s = miniexp_car((miniexp_t)expr);
    if (miniexp_consp(s) && miniexp_consp(miniexp_cdr(s)) && miniexp_stringp(miniexp_car(s)) && miniexp_stringp(miniexp_cadr(s)))
