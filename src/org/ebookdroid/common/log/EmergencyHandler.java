...
public class EmergencyHandler implements UncaughtExceptionHandler {
    ...
    private static File LOG_STORAGE;
    ...
    private void processException(final Throwable th) {
        try {
            ...
            final String name = EBookDroidApp.APP_PACKAGE + "." + EBookDroidApp.APP_VERSION + "." + timestamp
                    + ".stacktrace";
            final File filename = new File(LOG_STORAGE, name);
    ...
    private void writeToFile(final String stacktrace, final File filename) {
        try {
            ...
            bos.write("CPU_ABI     : " + EBookDroidApp.BUILD_PROPS.getProperty("ro.product.cpu.abi") + "\n");
            bos.write("CPU_ABI2    : " + EBookDroidApp.BUILD_PROPS.getProperty("ro.product.cpu.abi2") + "\n");
            bos.write("DEVICE      : " + Build.DEVICE + "\n");
            bos.write("DISPLAY     : " + Build.DISPLAY + "\n");
            bos.write("FINGERPRINT : " + Build.FINGERPRINT + "\n");
            bos.write("ID          : " + Build.ID + "\n");
            bos.write("MANUFACTURER: " + EBookDroidApp.BUILD_PROPS.getProperty("ro.product.manufacturer") + "\n");
    ...
    public static void init(final Context context) {
        if (system == null) {
            LOG_STORAGE = new File(EBookDroidApp.APP_STORAGE, "logs");
            LOG_STORAGE.mkdirs();
